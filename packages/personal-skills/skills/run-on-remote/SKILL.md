...
- **Use managed `nohup` jobs for long unattended commands.** Read `references/nohup.md` completely before starting one; it defines launch, status, logs, waiting, and result reporting. Keep foreground ssh for short commands whose output the user needs immediately.
...
- **Use a generous Bash timeout for foreground ssh**, 300000 ms or more. A timeout is observation policy, not persistence; it does not make a remote process survive a broken connection.
